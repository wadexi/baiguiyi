 ...
 * 为数据库映射提供Java类的数据类型。
 * <P>
 * 如果您要定义自己的自定义容器，那么您应该扩展{@link BaseDataType}。 看到
 * {@link DatabaseField＃persisterClass（）}。
 * </ p>
 * 
 * @author graywatson
...
public interface DataPersister extends FieldConverter {
	 ...
	 *
	 * 返回应与此关联的类名或null。 反射类使用它，所以我们可以
	 * 发现一个字段匹配_without_需要-core中的类依赖项。
	 */
	 ...
	 * 这使得数据类型的配置对象或如果没有则返回null。 稍后可以通过访问该对象
	 * {@link FieldType＃getDataTypeConfigObj（）}。
	 */
	 ...
	 * 如果数据库可以自动生成此类型，则返回true。 可能只有数字会返回true。
	 */
