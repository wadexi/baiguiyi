 ...
 * Spring wiring.
 * 数据库字段配置信息，由{@link DatabaseField}注释或直接Java或。提供
 * 弹簧布线。
 * 
 * @author graywatson
 */
public class DatabaseFieldConfig {
	...
	public static final Class<? extends DataPersister> DEFAULT_PERSISTER_CLASS = VoidType.class;//默认数据转换器
	public static final DataType DEFAULT_DATA_TYPE = DataType.UNKNOWN; //默认的数据类型
	...
	private String defaultValue;//字段注解设置的defaultvalue的值
	private int width; //通常用于string可以设置的最大长度
	private boolean canBeNull = DEFAULT_CAN_BE_NULL; //字段是否可以为空
	private boolean id; //当前字段是否为id ，对应数据库中的主键
	private boolean generatedId; //是否自动生成id
	private String generatedIdSequence;
	private boolean foreign; //对应数据库的外键， java中 bean存在非基本类，存在类成员
	private DatabaseTableConfig<?> foreignTableConfig; //非注解，没有注解的时候 set设置的值
	private boolean useGetSet;  //是否用getset方法获取 设置字段的值
	private Enum<?> unknownEnumValue; //数据库中的值不是任何一个枚举的名字，就用该值
	private boolean throwIfNull;
	private boolean persisted = true; //该字段是否和数据库字段映射，写入到数据库
	private String format; //格式化格式 例如日期
	private boolean unique; //不可重复的字段
	private boolean uniqueCombo; // 多个unique一起不可重复
	private boolean index; // 索引 。。。。。。
	private String indexName;// 索引 。。。。。。
	private boolean uniqueIndex;// 索引 。。。。。。
	private String uniqueIndexName;// 索引 。。。。。。
	private boolean foreignAutoRefresh;
	private int maxForeignAutoRefreshLevel = NO_MAX_FOREIGN_AUTO_REFRESH_LEVEL_SPECIFIED;
	private Class<? extends DataPersister> persisterClass = DEFAULT_PERSISTER_CLASS; //数据解析class类型
	private boolean allowGeneratedIdInsert;//允许自己创建id 插入
	 ...
	 * configuration.
	 *
	 * 对于不使用{@link DatabaseField}注释的外部类，您需要注入表
	 * 组态。
	 */
